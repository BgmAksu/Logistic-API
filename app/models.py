...
class Driver(Base):
    ...
    depot: Mapped[Optional[Depot]] = relationship(back_populates="drivers")
...
class Vehicle(Base):
    ...
    capacity_kg: Mapped[Optional[int]] = mapped_column(Integer)
    capacity_dm3: Mapped[Optional[int]] = mapped_column(Integer)
    depot_id: Mapped[Optional[int]] = mapped_column(ForeignKey("depots.id"))
...
# ---------- Shipments & parcels ----------
ShipmentStatus = Enum(
    "CREATED", "IN_TRANSIT", "DELIVERED", name="shipment_status"
)
...
class Shipment(Base):
    ...
    planned_delivery_date: Mapped[Optional[date]] = mapped_column(Date)
...
class Parcel(Base):
    ...
    weight_kg: Mapped[Optional[float]] = mapped_column(Double)
    volume_dm3: Mapped[Optional[float]] = mapped_column(Double)
...
class Stop(Base):
    ...
    planned_time: Mapped[Optional[datetime]] = mapped_column(DateTime)
    actual_time: Mapped[Optional[datetime]] = mapped_column(DateTime)
