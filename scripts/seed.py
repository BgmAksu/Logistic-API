    ...
    try:
        ...
        if db.scalar(select(func.count(models.Driver.id))) == 0:
            ...
            for d in depots:
                ...
                for i in range(2):
                    db.add(models.Vehicle(plate=f"{d.name[:3].upper()}-{fake.pyint(100,999)}",
                                          capacity_kg=1200, capacity_dm3=6000, depot_id=d.id))
            db.commit()
        ...
        if db.scalar(select(func.count(models.Route.id))) == 0:
            ...
            for a in random.sample(addr, 4):
                db.add(models.Stop(
                    route_id=r.id, sequence=seq, stop_type="DELIVERY",
                    address_id=a.id, planned_time=datetime.utcnow() + timedelta(minutes=15*seq)
                ))
                seq += 1
            db.commit()
